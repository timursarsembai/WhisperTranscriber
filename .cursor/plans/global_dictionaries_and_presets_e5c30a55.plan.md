---
name: Global Dictionaries and Presets
overview: Ввести глобальный пул словарей (общий для всех проектов), хранение в проекте только списка включённых словарей, типы correction/terms, пресеты наборов словарей и разделение prompt vs постобработка для исправлений.
todos: []
isProject: false
---
...
- **Вариант**: та же база, что и для конфига — в [i18n.py](i18n.py) используется `_config_path()` (рядом с exe или со скриптом). Добавить функцию вида `get_dictionaries_dir()` → `os.path.join(base, "dictionaries")`. При первом использовании создавать папку.
...
- **correction**: `entries` — массив `{ "original", "corrected" }` (как сейчас в [GlossaryService.py](GlossaryService.py)).
...
Вынести/расширить логику из [GlossaryService.py](GlossaryService.py) в новый или переименованный сервис (например `DictionaryService`), который:
...
- **Формирование prompt**:
  - `build_initial_prompt_text(dictionaries: List[DictionaryData])`:
    ...
    - из всех записей типа **correction** — секция «Use as written: original -> corrected» (как сейчас [get_initial_prompt_text](GlossaryService.py)).
...
В [SessionService.py](SessionService.py), [SessionData](SessionService.py):
...
В [main.py](main.py) при сохранении сессии передавать в `SessionService.build_session` текущий список включённых ID вместо одного `glossary_path`; при открытии — восстанавливать включённые словари из `enabled_dictionary_ids` (или из `glossary_path` при legacy).

---

## 5. Пресеты

- **Хранение**: глобально, не в проекте. Удобный вариант — в том же конфиге приложения, что и остальные настройки: в [i18n.py](i18n.py) `load_config`/`save_config` уже работают с `wi_config.json`. Добавить ключ, например `dictionary_presets`: массив `{ "name": "Подкасты", "enabled_ids": ["dict1.json", "dict2.json"] }`.
...
Переименовать вкладку и перестроить панель в [main.py](main.py) ([_build_glossary_panel](main.py) → например `_build_dictionaries_panel`):

1. **Блок «Глобальные словари»**
...
2. **Блок «Включено в этом проекте»**
...
Локализация: добавить ключи для «Словари», «Глобальные словари», «Включено в проекте», «Пресеты», «Применить», «Сохранить как пресет», «Применять исправления после транскрипции», «Добавить словарь», «Тип: исправления / подсказки» и т.д. в [locales](locales).
...
| Формат и загрузка/сохранение | [GlossaryService.py](GlossaryService.py) → расширить в DictionaryService или новый файл DictionaryService.py |
| Сессия                       | [SessionService.py](SessionService.py): SessionData.enabled_dictionary_ids, to_dict/from_dict, build_session |
| Транскрипция и prompt        | [main.py](main.py): вызовы transcribe, место после transcribe для apply_corrections_to_segments              |
| UI панели и пресеты          | [main.py](main.py): _build_glossary_panel → _build_dictionaries_panel                                        |
| Конфиг пресетов и опции      | [i18n.py](i18n.py) load_config/save_config или тот же wi_config.json                                         |
| Локализация                  | [locales/en.json](locales/en.json), ru, kk, es — новые ключи dictionaries.*                                  |
